...
impl<'sto> PduRx<'sto> {
    ...
    pub fn receive_frame(&mut self, ethernet_frame: &[u8]) -> Result<ReceiveAction, Error> {
        ...
        // 通过帧索引，返回原来的帧
        // Sent 状态的帧标记为 RxBusy 状态，返回 ReceivingFrame
        let mut frame = self
            .storage
        ...
        //i 包含了接收到的 EtherCAT数据报，将i中的数据复制到frame_data（帧缓冲区的帧）中
        frame_data
            .get_mut(0..i.len())
            .ok_or(Error::Internal)?
            .copy_from_slice(i);

        // 标记 RxBusy 帧的状态为 RxDone ，唤醒 帧 poll
        frame.mark_received()?;
