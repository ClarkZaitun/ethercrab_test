    ...
    S: Sync,
{
    ...
    // 添加一个从站到这个组
    unsafe fn push(&self, subdevice: SubDevice) -> Result<(), Error> {
        // get 方法返回一个指向其内部数据的可变原始指针 *mut GroupInner<MAX_SUBDEVICES>
        // 将传入的 SubDevice 实例添加到 subdevices 容器中
    ...
    // 将 SubDeviceGroup 转换为 SubDeviceGroupRef 类型，同时擦除其常量泛型参数
    fn as_ref(&self) -> SubDeviceGroupRef<'_> {
        // 创建并返回一个 SubDeviceGroupRef 实例， 擦除了 SubDeviceGroup 常量泛型参数
        SubDeviceGroupRef {
...
impl SubDeviceGroupRef<'_> {
    ...
    // 将组内的所有从设备（SubDevice）置于 PRE-OP状态，同时配置组内从设备在过程数据映像（PDI）中的映射。这里会修改pdi_position的start_address
    pub(crate) async fn into_pre_op<'sto>(
        ...
        // 设置当前组在 SubDeviceGroup::pdi 中的起始偏移量，单位字节
        *inner.pdi_start = pdi_position;
        ...
        // start_address 增加 max_pdi_len 字节，作为下一个组的起始偏移量
        // max_pdi_len 就是 MAX_PDI， 就是
        // TODO：给邮箱用还是FMMU用？
