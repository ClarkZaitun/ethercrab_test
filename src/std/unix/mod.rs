...
// ！！！ 这个文件内没有使用 await ，说明收发帧的操作可以只使用系统 API ，并不依赖异步运行时
    ...
    fn poll(mut self: Pin<&mut Self>, ctx: &mut core::task::Context<'_>) -> Poll<Self::Output> {
        ...
        match Pin::new(&mut self.socket).poll_read(ctx, &mut buf) {
            ...
            Poll::Ready(Ok(n)) => {
                ...
                // 确保本次任务执行完毕后还能再次唤醒当前任务，确保后续可能还有数据帧需要处理，特别是在 macOS 系统中，一次 poll_read 调用可能接收多个数据包，但这些数据包可能在下次 poll_read 时才返回
                ...
                // TODO：改为poll_read的buf长度和读取字节数不一致的错误
                ...
                // 过滤非EtherCAT帧，去除以太网帧头，EtherCAT头，通过index匹配发送的帧，得到EtherCAT数据区（数据报数组），将帧的状态切换为RxDone
                // 对非EtherCAT帧，也返回Ok(ReceiveAction::Ignored)
