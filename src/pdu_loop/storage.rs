...
pub struct PduStorage<const N: usize, const DATA: usize> {
    ...
    // 缓冲区中的被当前被使用的帧索引，用于遍历缓冲区
    // 初始化为0
    frame_idx: AtomicU8,
    // 数据报的index。初始化为0，后续用于递增，以区分不同的数据报
    pdu_idx: AtomicU8,
...
impl<const N: usize, const DATA: usize> PduStorage<N, DATA> {
    ...
    pub fn try_split(&self) -> Result<(PduTx<'_>, PduRx<'_>, PduLoop<'_>), ()> {
        ...
        Ok((
            // PduStorageRef 不可变引用可以有多个，因为它们不允许修改数据。
            // 通过内部可变性，可以修改 PduStorageRef 内部的状态。
...
pub(crate) struct PduStorageRef<'sto> {
    ...
    // frames指向的内存是包含的多个帧+帧描述符的一大块连续内存。后续如果想索引到某个帧，需要根据frame_element_stride计算帧在这块大内存的偏移量
...
impl<'sto> PduStorageRef<'sto> {
    ...
    pub(in crate::pdu_loop) fn alloc_frame(&self) -> Result<CreatedFrame<'sto>, Error> {
        ...
        // 我们已经搜索了两次，但没有找到空闲槽位。这意味着应用程序应该要么减慢数据包发送速度，要么增加 `PduStorage` 中的 `N` 值，因为
        // 没有足够的槽位来容纳所有正在传输的数据包。
